...
org.activemq.AmqClass = function() {
  ...
  var clientId = null;
	...
	var messageHandler = function(data) {
		...
		if (response != null && response.length == 1) {
			...
			for (var i = 0; i < responses.length; i++) {
				...
				//var id = responseElement.getAttribute('id');  TODO 临时修改id为chat，调试用  pengjianqiang
				var id = 'chat';
	...
	var pollErrorHandler = function(xhr, status, ex) {
		...
		if (status === 'error' && xhr.status === 0) {
			...
			tmrId = setTimeout(function() { sendPoll(); }, pollErrorDelay);
			return;
		}
	...
	var pollHandler = function(data) {
		try {
			...
			tmrId = setTimeout(sendPoll, pollDelay);
		}
	};
...
org.activemq.Amq = org.activemq.AmqClass();
