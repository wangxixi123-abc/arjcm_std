			   ...
			   //楼栋样式-fill透明、stroke透明、含有文字
		        'builds': new ol.style.Style({
							image : new ol.style.Icon({
										src : ctxStatic+ '/statis/images/'+ variable+ '',
...
//				}
		...
		addEventOverlayLabel:function(name, coordinate,overlayId) {
			...
			// 新建img图标标签
			var elementImg = document.createElement("img");
			elementImg.src = ctxStatic + "/statis/images/alarm.png";
			elementSpan.appendChild(elementImg);
		...
		addOverlayLabel3:function(name, coordinate,overlayId,icon) {
			...
			// 新建img图标标签
			var elementImg = document.createElement("img");
			elementImg.src = ctxStatic + "/statis/images/pub/"+icon;
			elementSpan.appendChild(elementImg);
...
//			}
		...
		trackReplayLu:function(startBtnId,speed,routeCoords,id,colorLine){
		    ...
		    var styles = {
		        ...
		        'startIcon': new ol.style.Style({
		            image: new ol.style.Icon({
		                anchor: [0.5, 1],
		                src: ctxStatic + '/statis/images/start.png'
		            })
		        }),
		        'endIcon': new ol.style.Style({
		            image: new ol.style.Icon({
		                anchor: [0.5, 1],
		                src:  ctxStatic + '/statis/images/end.png'
		            })
		        }),
		...
		addOverlayLabel4:function(name, coordinate,overlayId,icon,src) {
			...
			// 新建img图标标签
			var elementImg = document.createElement("img");
			elementImg.src = ctxStatic + "/statis/images/"+icon;
			elementSpan.appendChild(elementImg);
	  ...
	  //地图巡逻计划-删除旗帜标志 
		...
		selectPointer : function() {
			...
			var selectClick = new ol.interaction.Select({
			   ...
			   filter : function(feature, layer) {
				  return layer == _this['binguan']||_this['yule']||_this['shangchang']||_this['jiayouzhan']||_this['yiyuan']||_this['grids']||layer == _this['builds']||layer == _this['events']||layer == _this['parts']||layer == _this['lands']||layer == _this['videos']||layer == _this['keyPlace']||_this['xuexiao']||layer == _this['keyPerson']||layer == _this['rentingPerson']||layer == _this['publicPlace']||layer == _this['PopLocation']
									              ...
									              if(size==1){
									            	  ...
									            	  if(vectorArrType=="PopLocation"){
									                    ...
									                    style = styleCache[idIndex] = [
									                    	new ol.style.Style({
																	image : new ol.style.Icon({
																				src : ctxStatic+ '/statis/images/'+ Src+ '',
																				scale : map.getView().getZoom() / 15
																			}),
																	text: new ol.style.Text({
												                                textAlign: 'center', // 位置
