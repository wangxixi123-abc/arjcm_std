...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/org/ccmOrgNpseSpecial/">数据列表</a></li>
		<shiro:hasPermission name="org:ccmOrgNpseSpecial:edit">
			<li><a href="${ctx}/org/ccmOrgNpseSpecial/form">数据添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmOrgNpse"
		action="${ctx}/org/ccmOrgNpseSpecial/" method="post"
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmOrgNpse">
				<tr>
					...
					<td><a
						href="${ctx}/org/ccmOrgNpseSpecial/form?id=${ccmOrgNpse.id}">
					...
					<td><shiro:hasPermission name="org:ccmOrgNpseSpecial:edit">
							<a class="btnList"
								href="${ctx}/org/ccmOrgNpseSpecial/form?id=${ccmOrgNpse.id}"
								title="修改"><i class="icon-pencil"></i></a>
							<a class="btnList"
								href="${ctx}/org/ccmOrgNpseSpecial/delete?id=${ccmOrgNpse.id}"
								onclick="return confirmx('确认要删除该场所吗？', this.href)" title="删除"><i
								class="icon-trash"></i></a>
						</shiro:hasPermission> <shiro:hasPermission name="log:ccmLogTail:edit">
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npseSpecial', '记录信息', '800px', '660px')" 
								  title="记录信息"><i class="icon-print" style="color: cornflowerblue;"></i></a>
							<a class="btnList"
								onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npseSpecial', '添加记录', '800px', '660px')"
