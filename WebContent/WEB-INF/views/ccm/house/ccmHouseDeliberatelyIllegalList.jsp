...
<!-- 导入、导出模块 -->
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="width: 140px;text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatDeliberatelyIllegal">数据统计</a></li></shiro:hasPermission>
		<li class="active" style="width: 140px"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseDeliberatelyIllegal" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ccmHouseDeliberatelyIllegal" action="${ctx}/house/ccmHouseDeliberatelyIllegal/" method="post" class="breadcrumb form-search clearfix">
		...
		<ul class="ul-form pull-left">
			...
			<li class="first-line"><label>是否累犯：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
		...
		<tbody>
			<c:forEach items="${page.list}" var="ccmHouseDeliberatelyIllegal">
				<tr>
					<td width="100px"><img src="${ccmHouseDeliberatelyIllegal.images}" style="height:50px;" class="pimg"/></td>
					<td class="tc"><a onclick="parent.LayerDialog('${ctx}/house/ccmHouseDeliberatelyIllegal/form?id=${ccmHouseDeliberatelyIllegal.id}', '信息', '1200px', '900px')">${ccmHouseDeliberatelyIllegal.name}</a></td>
					<td class="tc">${ccmHouseDeliberatelyIllegal.ident}</td>
					<td class="tc">${fns:getDictLabel(ccmHouseDeliberatelyIllegal.sex, 'sex', '')}</td>
					<td class="tc">${ccmHouseDeliberatelyIllegal.telephone}</td>
					...
					<td class="tc">${fns:getDictLabel(ccmHouseDeliberatelyIllegal.isRecidivism, 'have_no', '')}</td>
					<td class="tc">${ccmHouseDeliberatelyIllegal.convictCrimes}
					</td>
					<td class="tc"><shiro:hasPermission name="house:ccmHouseDeliberatelyIllegal:edit">
						...
						<a class="btnList" href="${ctx}/house/ccmHouseDeliberatelyIllegal/delete?id=${ccmHouseDeliberatelyIllegal.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该故意犯法释放人口吗？', this.href)" title="删除"><i class="icon-remove-sign"></i></a>
						<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseDeliberatelyIllegal.peopleId}')"  title="位置信息"><i class="icon-map-marker "></i></a>
						...
						<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseDeliberatelyIllegal.id}&relevance_table=ccm_house_deliberatelyIllegal', '记录信息', '800px', '660px')" 
