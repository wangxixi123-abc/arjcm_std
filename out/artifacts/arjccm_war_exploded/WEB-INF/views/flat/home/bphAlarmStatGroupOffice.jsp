...
<div class="clearfix" style="width:100%;height:100%">
  <div class="" style="float:left;width:330px;height:100%; border-right: 1px solid #ccc;">
   ...
   <div class="right-conent" style="float:left;height:100%">
     <div id="officeStatAlarm" style="width:100%;height:100%"></div>
   </div>
</div>
...
var alarmData=[];
<c:forEach items="${typeStat.data}" var="data">
alarmData.push({
		value:'${data.num}', name:'${data.name}'
})
</c:forEach>
var officeStat=[],legendName=[];
<c:forEach items="${officeStat.data}" var="data">
officeStat.push({
	name:'${data.officeName}', value:'${data.num}',officeId:'${data.officeId}'
})
legendName.push('${data.officeName}')
</c:forEach>
var FontColor='#fff';
...
var option = {
	   ...
	   // 实例化 对象
	var Piechart = echarts.init(document.getElementById('officeStatAlarm'));
