...
//			}
		...
		addOverlayLabel4:function(name, coordinate,overlayId,icon,src) {
			...
			// 新建span标签
			var elementSpan = document.createElement("a");
			elementTd1.appendChild(elementSpan);
			// 新建img图标标签
		...
		addJSON1:function(params){
			...
			if (len > 0) {
				for (var i = 0; i < len; i++) {
					...
					console.info("111111111",vectorArr[i].data);
					//数据
					vectorSource=this.vectorGeoJSON(vectorArr[i].data)
					//点聚合
					console.info("22222222222",vectorSource);
					clusterSource=this.cluster(vectorSource);
					 if(DataLen>0){
						 console.info("33333333",clusterSource);
							for(var j=0;j<DataLen;j++){
								if(Data[j].geometry.type=="Point"){
					 ...
					 console.info("444444444",layerVectortype);
					var styleCache = {};
					var cont1=0;
...
//										//_this.addOverlayLabel(len,coordinates,id)
									...
									//返回样式
									console.log("6485455--64",_this.featureStyles(iconSrc,feature)[vectorArrType]);
									if(_this.featureStyles(iconSrc,feature)[vectorArrType]){
										...
										return  _this.featureStyles(iconSrc,feature)[vectorArrType];
											...
											if(GeometryType=="Point"){
									            ...
									            _this[vectorArrType].setZIndex(20);
									            return style;
											}else{
					...
					console.info("--",this[vectorArrType]);
...
//												                            })
		...
		drawPoint:function(data){
			...
			var map = this.map;
			var beijing = ol.proj.fromLonLat([117.66768872737885, 39.04421389102936]);
	        ...
	        function createLabelStyle(feature){
	            ...
	            return new ol.style.Style({
	                image: new ol.style.Icon({
	                    ...
	                    opacity: 0.75,
	                    src: 'http://openlayers.org/en/v3.9.0/examples/data/icon.png'  //图标的URL
	                }),
	                text: new ol.style.Text({
	                    textAlign: 'center',            //位置
	                    textBaseline: 'middle',         //基准线
