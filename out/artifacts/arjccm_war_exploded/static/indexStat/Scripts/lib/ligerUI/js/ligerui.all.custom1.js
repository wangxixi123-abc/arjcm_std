...
(function ($)
{
    ...
    //dialog 图片文件夹的路径 预加载
    $.ligerui.DialogImagePath = "~/Script/lib/ligerUI/skins/Aqua/images/win/";
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        onfield: 'Online',                      //在线数
        offfield:'Offline'                      //离线数
    };
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '显示从{from}到{to}，在线数: {online}，离线数: {offline}, 总 {total} 条 。每页显示：{pagesize}',
        ...
        */
        ...
        _showData: function (sourceType)
        {
            ...
            if (p.usePager)
            {
                ...
                if (!p.page) p.page = 1;
                p.pageCount = Math.ceil(p.total / p.pageSize);
                p.online = g.data[p.onfield];
                p.offline = g.data[p.offfield];
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        //gjf
                        var startindex = parseInt((p.page - 1) * p.pageSize);
                        gridhtmlarr.push('>' + (startindex + parseInt(item['__index']) + 1) + '</div></td>');
                        return;
                    }
                    //如果是复选框(系统列)
                    if (this.ischeckbox)
        ...
        _buildPager: function ()
        {
            ...
            if (r1 < 0) r1 = 0;
            if (r2 < 0) r2 = 0;      
            if (!p.online) p.online = 0;
            if (!p.offline) p.offline = 0;
            var stat = p.pageStatMessage;
            stat = stat.replace(/{from}/, r1);
            stat = stat.replace(/{to}/, r2);        
            stat = stat.replace(/{online}/, p.online);
            stat = stat.replace(/{offline}/, p.offline);
