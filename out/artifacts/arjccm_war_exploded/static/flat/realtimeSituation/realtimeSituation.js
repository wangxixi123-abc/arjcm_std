...
function getCaseType(){
	$.getJSON(ctx+'/sys/dict/listData?type=bph_alarm_info_typecode',function(data){
		var len=data.length;
		if(len>0){
			var html='';
			for(var i=0;i<len;i++){
				html+='<div class="checkbox checkbox-success radio-div tab-content-bg" style="padding-top: 5px;padding-bottom: 5px;">';  
...
function okFun(){
	...
	if(type=='1'){//热力
		$.getJSON(ctx+'/alarm/bphAlarmInfo/queryAlarmInfoByDateAndAlarmType?beginAlarmTime='+beginTime+'&endAlarmTime='+endTime+'&typeCode='+id+'',function(data){
		...
		$.getJSON(ctx+'/alarm/bphAlarmInfo/queryAlarmInfoByDateAndAlarmTypeToFourColor?beginAlarmTime='+beginTime+'&endAlarmTime='+endTime+'&typeCode='+id+'',function(data){
			if(data==null||data==""||data==undefined){
            	 $.jBox.tip('暂无数据！');
                 return ;
             }
