...
<div class="back-list">
	...
	<form:form id="searchForm" modelAttribute="plmIncomingEntry"
		action="${ctx}/storage/plmOut/" method="post"
		class="breadcrumb form-search clearfix">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden"
			value="${page.pageSize}" />
		<ul class="ul-form pull-left">
			<li class="first-line"><label>出库开始日期：</label>
			...
			<li class="first-line"><label>出库结束日期：</label>	<input name="endIncomingDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${plmIncomingEntry.endIncomingDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>
			</li>
			<li class="first-line"><label>出库单号：</label> <form:input path="incomingCode"
					htmlEscape="false" maxlength="64" class="input-medium" /></li>
			<li class="first-line"><label>出库类别：</label> <form:select path="incomingType"
					class="input-small">
					<form:option value="" label="未选择" />
					<form:options items="${fns:getDictList('plm_out_type')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<li class="first-line"><label>出库状态：</label> <form:select path="type"
					class="input-medium">
					<form:option value="" label="未选择" />
					<form:options items="${fns:getDictList('plm_out_status')}"
						itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select></li>
			<%--<li class="clearfix second-line"></li>--%>
		...
		<div class="row">
			<c:forEach items="${page.list}" var="plmOut">
				<div class="span4 spandiv">
					<div class="thumbnail">
						<a href="${ctx}/storage/plmOut/form?id=${plmOut.id}">

							<h4 title="${plmOut.incomingCode}">出库单号:${plmOut.incomingCode}</h4>
						</a> </a>
						<div class="caption row-fluid">
							...
							<div class="spantext  " style="width: 63%; margin-left: 7%">
								<p
									title="${fns:getDictLabel(plmOut.incomingType, 'plm_out_type', '未知')}">出库类别:${fns:getDictLabel(plmOut.incomingType, 'plm_out_type', '未知')}</p>
								<p
									title="${fns:getDictLabel(plmOut.type, 'plm_out_status', '未知')}">出库状态:${fns:getDictLabel(plmOut.type, 'plm_out_status', '未知')}</p>
								<p
									title="<fmt:formatDate value="${plmOut.incomingDate}" pattern="yyyy-MM-dd"/>">
									出库日期:
