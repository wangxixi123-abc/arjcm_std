...
<body>
	...
	<div id="content" class="row-fluid">
		...
		<div id="right">
			<iframe id="storageContent" src="${ctx}/act/plmAct/mylist?status=04&procInsIds=" width="100%" height="91%" frameborder="0"></iframe>
		</div>
	</div>
	<script type="text/javascript">
		var setting = {view: {addDiyDom : textDom},data:{simpleData:{enable:true,idKey:"id",pIdKey:"pId",rootPId:''}},
			callback:{onClick:function(event, treeId, treeNode){
					...
					if(procInsId!=null&&procInsId!=""){
						$('#storageContent').attr("src","${ctx}/act/plmAct/mylist?status=04&procInsIds="+procInsId);
					}
					return;
				}
			}
		...
		function refreshTree(){
			$.getJSON("${ctx}/sys/dict/treeData4?status=04",function(data){
