...
<c:forEach items="${percentData}" var="data">
	alarmData.push({
		value:'${data.num}', name:'${data.office.name}',typeCode:nowTypeCode
	})
</c:forEach>
var countData=[];
var type='${type}';
...
var dateData=[];
<c:forEach items="${countData}" var="data">
countData.push('${data.num}')
...
Piechart.on('click', function (params) {
     ...
     window.location.href="${ctx}/alarm/bphAlarmInfo/statPercentByAlarmType?typeCode="+typeCode
});
// 填充数据
