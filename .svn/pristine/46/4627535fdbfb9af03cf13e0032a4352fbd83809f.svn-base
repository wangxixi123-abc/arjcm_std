$(function() {
	...
	// 标题
	var title1 = [ "本月特殊关怀新增人数", "本月特殊关怀总数" ], title2 = [ "新增特殊关怀人数", "特殊关怀总人数" ];
	// 初始化页面方法
	showEchart();
	// 展示 页面的方法
	function showEchart() {
		...
		$.post(context + "/report/ccmPeopleStat/personStat", {
			...
			// 生成图表
			$.myChartPeople(title1, ajaxDataX1, seriesDate1, myChart1,"特殊关怀人数",40,areaToContent);
			$.myChartPeople(title2, ajaxDataX2, seriesDate2, myChart2,"特殊关怀人数",0,optionToContent);
			// 生成 表单
