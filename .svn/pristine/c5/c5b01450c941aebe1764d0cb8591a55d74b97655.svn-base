		...
		$("#btnExport").click(
				function() {
					top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function(v, h, f) {
						if (v == "ok") {
							// 借用导出action
							$("#searchForm").attr("action", "${ctx}/sys/ccmWorkReport/exportVisit");
							$("#searchForm").submit();
							// 还原查询action 
							$("#searchForm").attr("action", "${ctx}/sys/ccmWorkReport/Visit");
						}
					}, {
						buttonsFocus : 1
					});
...
<ul class="back-list">
	<ul class="nav nav-tabs">
		<li class="active" style="width: 112px"><a class="nav-head" href="${ctx}/sys/ccmWorkReport/Visit/">数据列表</a></li>

	</ul>
	<form:form id="searchForm" modelAttribute="ccmWorkReport"
		action="${ctx}/sys/ccmWorkReport/Visit/" method="post"
