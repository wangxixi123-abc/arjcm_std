...
<body>
	<form:form id="searchForm" modelAttribute="plmEquApply"
		action="${ctx}/equapply/plmEquApplyMaintenance/" method="post"
		...
		<table id="contentTable"
			...
			<tbody>
				<c:forEach items="${page.list}" var="plmEquApply">
					<tr>
						<td><a
							href="${ctx}/equapply/plmEquApplyMaintenance/form?id=${plmEquApply.id}">${plmEquApply.user.name}
						</a></td>
						...
						<td>
								...
								<c:if test="${ empty plmEquApply.procInsId}">
									<a class="btnList"
										href="${ctx}/equapply/plmEquApplyMaintenance/form?id=${plmEquApply.id}"
					...
					<div class="thumbnail">
						...
						<div class="caption row-fluid">
							...
							<div class="spantext  " style="width: 84%; margin-left: 7%">
								<p title="${plmEquApply.user.name}">申请人:${plmEquApply.user.name}</p>
