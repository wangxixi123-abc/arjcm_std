...
/**
 * 拦截查询SQL，根据分页对象自动设置参数
 */
@Intercepts({ @Signature(type = Executor.class, method = "query", args = { MappedStatement.class, Object.class,
		RowBounds.class, ResultHandler.class }) })

public class AuthorityInterceptorCmm extends BaseInterceptor {
			...
			if (role != null) {
				if (role.equals("新沂管理员") || role.equals("系统管理员")) {
					...
					if (originalSql.toLowerCase().indexOf("checker_userid") != -1) {
						String usersSql = "select a.userid from tbldepartment_user a where a.departmentcode=(select b.departmentcode from tbldepartment_user b where b.userid='"
								+ check_user + "')";
						String riskSql = "select a.* from (" + originalSql + ")a  where a.checker_userid in ("
								+ usersSql + ")";
						...
						// ParameterMapping.Builder(mappedStatement.getConfiguration(),
						// check_user, String.class).build();
